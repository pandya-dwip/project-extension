...
| **Release Management** | Log releases with version, owner, work items, and notes; advance through a seven-stage status workflow (Draft → Released); generate email-style release announcements in one click; copy notes to clipboard. |
| **Release Points** | Track multi-project release checkpoints with completion progress bars and a timeline view. |
...
| Release Management | `releases` | Release lifecycle and note generation |
| Release Points | `releasepoints` | Multi-project release progress timeline |
...
The **Generate Template** button fills the notes field with a formatted release announcement. The **Copy** button writes the saved notes to the clipboard; if notes are empty it generates fallback text from the release fields.
...
```
project-extension/
├── manifest.json            Chrome MV3 metadata, permissions, service worker declaration
├── background.js            Service worker — tab lifecycle management
├── index.html               Static shell: sidebar, top bar, modals, form controls
├── app.js                   Single-page controller (~7 000 lines)
├── style.css                Complete stylesheet with CSS custom properties (~5 300 lines)
...
3. Test both **extension mode** and **standalone `index.html` mode** after any storage or persistence change.
4. Keep storage shape changes backward-compatible with existing backup files where possible.
5. Update this README when adding views, form fields, storage collections, Chrome permissions, or browser APIs.
...
- [ ] Release version dropdown populates from the selected project's version fields.
